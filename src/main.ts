...
import { signUp, signIn, signOut, getCurrentUser, getUserProfile } from './lib/auth';
import {
  createUserProfile,
  updateUserProfile,
  getAllUserProfiles,
...
function populatePlayers() {
  ...
  if (select) {
    ...
    populateProps(); // Initialize props for first player
  }
}
...
function updatePotentialPayout() {
  ...
  Array.from(container.children).forEach(prop => {
    ...
    const odds = 4; // Fixed odds
    total += amount * odds;
    totalBet += amount;
    
    // Update color based on bet amount
    if (amount > 0) {
  ...
  if (payoutDisplay) {
    payoutDisplay.textContent = `${total} (Betting: ${totalBet})`;
    payoutDisplay.style.color = total > 0 ? '#00ff00' : '#888';
  }

  // Update submit button state
  const submitBtn = document.getElementById('submit-bets') as HTMLButtonElement;
  if (submitBtn) {
    submitBtn.disabled = totalBet === 0 || totalBet > (currentProfile?.available_coins || 0);
  }
}
...
async function handleSubmitBets() {
  ...
  if (totalAmount > currentProfile.available_coins) {
  ...
  try {
    ...
    for (const bet of bets) {
      await placeBet(currentUser.id, bet.player, bet.prop, bet.amount, bet.odds);
    }

    // Update available coins
    currentProfile.available_coins -= totalAmount;
    ...
    history.forEach(record => {
      ...
      if (Array.isArray(record.bets_data)) {
        record.bets_data.forEach((bet: any) => {
          const color = bet.won ? '#00ff00' : '#ff0000';
          const result = bet.won ? `Won ${bet.amount * bet.odds}` : `Lost ${bet.amount}`;
          div.innerHTML += `<span style="color:${color}">${bet.player} - ${bet.prop} - Bet: ${bet.amount} - ${result}</span><br>`;
        });
      }
...
async function handleStartMatch() {
  ...
  try {
    ...
    updateLeaderboards();
    alert("Match started! Users can now vote for MVP.");
  } catch (error: any) {
...
async function handleEndMatch() {
  ...
  if (!confirm("End match and resolve all bets?")) return;
  ...
  try {
    ...
    updateLeaderboards();
    updateHistory();
    alert("Match ended! All bets have been resolved.");
  } catch (error: any) {
